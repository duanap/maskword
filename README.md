...
- 刷新/短暂断线恢复；房主离线 15 秒后自动转移，也可在任何阶段手动转移
...
V1 不包含账号、语音、录音、文字聊天、广告、数据库、Redis、ORM 和 Docker。
...
线上联机以服务端为唯一可信状态源，房间状态只保存在 Node.js 进程内存中；服务重启后现有房间会失效。线下同屏不会创建房间或连接 Socket.IO，状态保存在浏览器的 `maskword-offline-v1` 本地存储中。共享词库位于 `packages/shared/src/words.ts`。

## 线下同屏

- 首位现场成员默认是主持人；主持人可参赛或仅主持，也可在游戏中转移权限
- 默认身份配置按人数自动推荐，5 人起包含 1 名白板，7 人起增加卧底；高级设置仍需满足开局约束
...
测试覆盖共享身份规则、白板首位保护、线上隐私快照、弃权和平票重投、房主淘汰后主持、恢复令牌、主动退出，以及线下三人完整对局、不参赛主持、本地恢复、整轮重投和再来一局。
